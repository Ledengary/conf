...
from general import load_wconf_dataset, load_few_shot_examples, load_system_prompt, flatten_dataset, assert_flatten_dataset, get_last_hidden_state_size, initialize_tokenizer, create_collate_fn, setup_data_loaders
...
from confhead import ConfidenceHeadFlattenInput


def setup_model_and_optimizer(hidden_size, device):
    confidence_head = ConfidenceHeadFlattenInput(hidden_size)
...
def main(dataset_name, llm_id, wconf_dir_path, postfix, shots_dir_path, nshots, max_seq_length, epochs, gpu_memory, tensor_parallel, vllm_task, batch_q, nruns, seed):
...
# python contrastive-proj-train.py --dataset_name "GSM8K" --llm_id "neuralmagic/Meta-Llama-3.1-8B-Instruct-quantized.w8a8" --wconf_dir_path "../data/GSM8K/Meta-Llama-3.1-8B-Instruct-quantized.w8a8/wid" --postfix "_wconf_wid" --shots_dir_path "../data/GSM8K/" --nshots 10 --visible_cudas "5" --gpu_memory 0.8 --tensor_parallel 1 --max_seq_length 1024 --vllm_task "reward" --epochs 5 --batch_q 1 --nruns 30 --seed 23